...
def compute_properties(q,p):
   
    ndof, ntraj = q.num_of_rows, q.num_of_cols

    expect_q, expect_p, expect_q2, expect_p2 = 0.0, 0.0, 0.0, 0.0
    for i in xrange(ndof):
        for j in xrange(ntraj):

            expect_q  += q.get(i,j)
            expect_p  += p.get(i,j)
            expect_q2 += q.get(i,j)*q.get(i,j)
            expect_p2 += p.get(i,j)*p.get(i,j)

    expect_q  /= ntraj; expect_p  /= ntraj
    expect_q2 /= ntraj; expect_p2 /= ntraj

    return expect_q, expect_p, expect_q2, expect_p2
